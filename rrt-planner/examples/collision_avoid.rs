...
use path_plan::{Space, RandomSample};
use path_plan::dual_rrt_connect;
...
impl Space for CollisionProblem {
...
impl RandomSample for CollisionProblem {
...
fn main() {
    ...
    while window.render() {
        if index == path.len() {
            path = dual_rrt_connect(&start, &goal, 0.05, &p, 1000).unwrap();
            index = 0;
        }
